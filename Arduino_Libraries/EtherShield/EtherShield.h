...
class EtherShield
{
  public:
	...
	void ES_send_udp(uint8_t *buf,char *data,uint8_t datalen,uint16_t sport, uint8_t *dip, uint16_t dport, uint8_t *udpmac);
	...
	void ES_fill_buf_p(uint8_t *buf,uint16_t len, const char * const progmem_s);
...
//	void ES_dnslkup_request(uint8_t *buf, const char * hoststr);
...
#define HTTP_HEADER_START ((uint16_t)TCP_SRC_PORT_H_P+(buf[TCP_HEADER_LEN_P]>>4)*4)
#ifdef WWW_client
	// ----- http get
	void ES_client_browse_url(const char *urlbuf, char *urlbuf_varpart, const char *hoststr,
	...
	void ES_client_http_post(const char *urlbuf, const char *hoststr, const char *additionalheaderline,
			const char *method, char *postval,void (*callback)(uint8_t,uint16_t,uint16_t));
	// The callback is a reference to a function which must look like this:
	// void browserresult_callback(uint8_t statuscode,uint16_t datapos)
	// statuscode=0 means a good webpage was received, with http code 200 OK
	// statuscode=1 an http error was received
...
//#endif // PING_client
    ...
    uint8_t ES_enc28j60GetLastError(void);
};
