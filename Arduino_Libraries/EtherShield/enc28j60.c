...
static uint8_t WaitUntilPhyFinished(void);
...
uint8_t enc28j60GetLastError( void )
{
    uint8_t retVal = lastError;
    lastError = 0;
    return retVal;
}
...
static uint8_t WaitUntilPhyFinished(void)
{
    ...
    while( (enc28j60Read(MISTAT) & MISTAT_BUSY) && --timeout)
    {
        delayMicroseconds(15);
    }
...
void enc28j60InitWithCs( uint8_t* macaddr, uint8_t csPin )
{
	...
	// See Rev. B1 B4 Point 1 Errata states to use at least 8MHz SPI
	// The ENC28J60 is specified to work at up the 20MHz
	...
	// 0 1 0 fosc/64
	// 0 1 1 fosc/128
	// 1 0 0 fosc/2   -> 8MHz at 16MHz clock
	// 1 0 1 fosc/8
	// 1 1 0 fosc/32
	// 1 1 1 fosc/64
...
//  enc28j60WriteOp(ENC28J60_BIT_FIELD_SET, MACON3, MACON3_PADCFG2|MACON3_PADCFG1|MACON3_PADCFG0|MACON3_TXCRCEN|MACON3_FRMLNEN);
    ...
    // enable interrutps
    enc28j60WriteOp(ENC28J60_BIT_FIELD_SET, EIE, EIE_INTIE|EIE_PKTIE|EIE_TXERIE|EIE_RXERIE);
...
{
    // Check no transmit in progress and wait until finished
    // We only have enough buffer space for one frame
    unsigned int timeout = 100;
    while ( (enc28j60ReadOp(ENC28J60_READ_CTRL_REG, ECON1) & ECON1_TXRTS) && --timeout )
    {
       ...
       || (enc28j60Read(ECON1) & (ECON1_TXRST)) )
		{
...
uint16_t enc28j60PacketReceive(uint16_t maxlen, uint8_t* packet)
{
	...
	uint8_t tmpEIR = enc28j60Read(EIR);
    if(   (tmpEIR & (EIR_RXERIF))
       || (enc28j60Read(ECON1) & (ECON1_RXRST))
       )
    {
      lastError = 5;
    }
    if(   (tmpEIR & (EIR_TXERIF))
       || (enc28j60Read(ECON1) & (ECON1_TXRST))
      )
    {
      lastError = 6;
    ...
    // Reset the transmit logic problem. See Rev. B4 Silicon Errata point 12.
    if(   (enc28j60Read(EIR) & EIR_TXERIF)
       || (enc28j60Read(ECON1) & (ECON1_TXRST|ECON1_RXRST)) )
    {
      enc28j60WriteOp(ENC28J60_BIT_FIELD_SET, ECON1, ECON1_TXRST);
      enc28j60WriteOp(ENC28J60_BIT_FIELD_CLR, ECON1, ECON1_TXRST);
    }
