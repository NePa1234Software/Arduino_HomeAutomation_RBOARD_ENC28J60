...
static const char *client_method;  // for POST or PUT, no trailing space needed
...
void fill_buf_p(uint8_t *buf,uint16_t len, PGM_P progmem_s )
{
        while (len){
...
void send_udp_prepare(uint8_t *buf,uint16_t sport, uint8_t *dip, uint16_t dport, uint8_t *udpmac)
{
        uint8_t i=0;
        //
        while(i<6){
                buf[ETH_DST_MAC +i]=udpmac[i]; // gw mac in local lan or host mac
                buf[ETH_SRC_MAC +i]=macaddr[i];
                i++;
        }
        buf[ETH_TYPE_H_P] = ETHTYPE_IP_H_V;
...
void send_udp(uint8_t *buf,char *data,uint8_t datalen,uint16_t sport, uint8_t *dip, uint16_t dport, uint8_t *udpmac)
{
        send_udp_prepare(buf,sport, dip, dport, udpmac);
        uint8_t i=0;
        // limit the length:
        if (datalen>220){
                datalen=220;
...
//
//
void client_browse_url(PGM_P urlbuf, char *urlbuf_varpart, PGM_P hoststr, void (*callback)(uint8_t,uint16_t,uint16_t))
{
...
void client_http_post(PGM_P urlbuf, PGM_P hoststr, PGM_P additionalheaderline, const char * method, char *postval,void (*callback)(uint8_t,uint16_t,uint16_t))
{
        client_urlbuf=urlbuf;
        client_hoststr=hoststr;
