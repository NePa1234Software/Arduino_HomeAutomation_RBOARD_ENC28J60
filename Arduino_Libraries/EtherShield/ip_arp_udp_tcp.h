...
extern void fill_buf_p(uint8_t *buf,uint16_t len, PGM_P progmem_s);
...
#ifdef WWW_client
// ----- http get
extern void client_browse_url(PGM_P urlbuf, char *urlbuf_varpart, PGM_P hoststr, void (*callback)(uint8_t,uint16_t,uint16_t));
...
extern void client_http_post(PGM_P urlbuf, const char *hoststr, PGM_P additionalheaderline, const char *method, char *postval,void (*callback)(uint8_t,uint16_t,uint16_t));
// The callback is a reference to a function which must look like this:
// void browserresult_callback(uint8_t statuscode,uint16_t datapos)
// statuscode=0 means a good webpage was received, with http code 200 OK
// statuscode=1 an http error was received
...
extern void send_udp(uint8_t *buf,char *data,uint8_t datalen,uint16_t sport, uint8_t *dip, uint16_t dport, uint8_t *udpmac);
#endif
